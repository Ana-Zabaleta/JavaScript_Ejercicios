...
function filtrarStreamers(event) {
  ...
  const filtrarStreamers = streamers.filter((streamer) => {
    const streamerNombre = streamer.name.toLowerCase();
    return streamerNombre.includes(keyword);
  });

  console.log(filtrarStreamers);
}

document
  .querySelector("input[data-function='toFilterStreamers']")
  .addEventListener("input", filtrarStreamers);
